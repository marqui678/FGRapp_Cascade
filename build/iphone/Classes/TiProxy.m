...
//Rare exceptions to indicate a bug in the _fgr_cascade code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_fgr_cascade = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_fgr_cascade]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.fgr_cascade.module"
